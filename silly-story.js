...
/* STEP 6: Review the partially complete result() function below */
 ...
 //Assuming you have already defined the randomValueFromArray function and the insertX, insertY, and insertZ arrays

  // Call the function for each variable and pass the respective array
 ...
 var zItem = randomValueFromArray(insertZ);
    ...
    // Assuming you have defined customName and newStory

    // Check if customName is not empty
    if (customName.trim() !== "") {
    // Replace 'Bob' with the user-entered name in newStory
    ...
    newStory = newStory.replace(/(\d+) fahrenheit/gi, function(match, p1) {
        return Math.round((parseInt(p1) - 32) * 5/9) + " Celsius";
    });

    // Convert weight from pounds to kilograms
    newStory = newStory.replace(/(\d+) pounds/gi, function(match, p1) {
        return Math.round(parseInt(p1) * 0.453592) + " kilograms";
    });
